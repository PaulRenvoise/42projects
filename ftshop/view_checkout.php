...
function fetch_checkout($db, $id)
{
	...
	$statement	= "SELECT `checkouts`.*, `checkouts_articles`.`id` AS `article_id`, `checkouts_articles`.`price` AS `article_price`, `checkouts_articles`.`quantity` AS `article_quantity`, `articles`.`name` AS `article_name` FROM `checkouts`
		...
		WHERE `checkouts`.`user_id` = '$user_id' AND `checkouts`.`id` = '$id'";
...
if (!empty($_SESSION['user_id']) && is_numeric($_GET['id']))
{
	$error				= '';
	$id					= intval($_GET['id']);
...
<p class="checkout_infos">
	...
	<span>Statut : </span><?= $checkout['status'] ?><br><br>
...
<p class="right">
	<a href="account.php">Retour au panneau utilisateur</a>
</p>
<?php endif ?>
<?php
